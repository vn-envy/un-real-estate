...
export default function App(){
  ...
  return(<div style={{minHeight:"100vh",background:"#FAF9FD",color:"#1a1c1e",fontFamily:"'Inter',system-ui,sans-serif",overflowX:"hidden"}}>
    ...
    <style>{`@keyframes fadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}@keyframes morphIn{from{opacity:0;transform:scaleY(.3)}to{opacity:1;transform:scaleY(1)}}@keyframes glowP{0%,100%{box-shadow:0 0 20px rgba(82,45,230,.15)}50%{box-shadow:0 0 40px rgba(82,45,230,.3)}}@keyframes cashP{0%,100%{opacity:.6}50%{opacity:1}}.anim{animation:fadeUp .4s ease both}.morph{animation:morphIn .6s cubic-bezier(.16,1,.3,1) both}.glow{animation:glowP 2s ease infinite}.cashB{animation:cashP 1.5s ease infinite}@keyframes spin{to{transform:rotate(360deg)}}.glass{background:rgba(250,249,253,.82);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px)}.lift{box-shadow:0 24px 48px -12px rgba(98,95,123,.1)}.ghost{border:1px solid rgba(201,196,217,.25)}input[type=number]::-webkit-inner-spin-button{-webkit-appearance:none}input[type=number]{-moz-appearance:textfield}button{-webkit-tap-highlight-color:transparent}*{-webkit-overflow-scrolling:touch}`}</style>

    <header className="glass" style={{position:"fixed",top:0,width:"100%",zIndex:50,borderBottom:"1px solid rgba(201,196,217,.15)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 16px",maxWidth:540,margin:"0 auto",gap:8}}>
        <div style={{minWidth:0}}><div style={{fontSize:"clamp(.84rem,3.5vw,1rem)",fontWeight:900,letterSpacing:"-.04em",fontFamily:"'Plus Jakarta Sans'",whiteSpace:"nowrap"}}><span style={{color:"#522de6"}}>un</span>-real-estate</div><div style={{fontSize:".46rem",color:"#787587",whiteSpace:"nowrap"}}>the real cost, before you sign</div></div>
        {phase==="results"&&<button onClick={()=>{setPhase("input");setDetailIdx(null)}} style={{padding:"6px 12px",borderRadius:99,background:"#f4f3f7",fontSize:".68rem",fontWeight:700,color:"#787587",border:"none",cursor:"pointer",flexShrink:0}}>Edit</button>}
      </div>
    </header>

    <main style={{maxWidth:540,margin:"0 auto",padding:"64px 16px 80px",overflowX:"hidden"}}>
      {/* INPUT */}
      ...
      {phase==="results"&&R&&<div ref={pyrRef}>
        ...
        {/* TABS */}
        <div style={{display:"flex",gap:3,marginBottom:14,overflowX:"auto",paddingBottom:4,WebkitOverflowScrolling:"touch"}}>
          {[{k:"pyramid",l:"Breakdown"},{k:"timeline",l:"Timeline"},{k:"rent",l:"Buy vs Rent"},{k:"intel",l:"Intelligence"}].map(t=><button key={t.k} onClick={()=>{setTab(t.k);setDetailIdx(null)}} style={{padding:"7px 12px",borderRadius:9,border:tab===t.k?"1.5px solid #522de6":"1px solid #e3e2e6",background:tab===t.k?"#522de610":"#fff",color:tab===t.k?"#522de6":"#787587",fontSize:".68rem",fontWeight:700,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>{t.l}</button>)}
        </div>

        {/* PYRAMID TAB */}
        {tab==="pyramid"&&<div className="anim">
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,marginBottom:18,padding:"0 4px"}}>
            {R.sorted.map((seg,i)=>{const w=100-(i*12);const isO=detailIdx===i;const hasCash=seg.id==="upfront"&&R.tCash>0;
              return <div key={seg.id} className="morph" style={{width:`${w}%`,maxWidth:"100%",animationDelay:`${i*.08}s`}}>
                <button onClick={()=>setDetailIdx(isO?null:i)} style={{width:"100%",padding:i===0?"14px 12px":"10px 10px",borderRadius:i===0?"4px 4px 14px 14px":i===R.sorted.length-1?"12px 12px 4px 4px":5,background:seg.color,color:"#fff",border:isO?"2px solid #fff":"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",gap:6,boxShadow:`0 4px 14px ${seg.color}30`,transition:"all .3s",transform:isO?"scale(1.02)":"scale(1)"}}>
                  <div style={{display:"flex",alignItems:"center",gap:5,minWidth:0,overflow:"hidden"}}>
                    <span className="material-symbols-outlined" style={{fontSize:i===0?16:14,fontVariationSettings:"'FILL' 1",flexShrink:0}}>{seg.icon}</span>
                    <span style={{fontSize:i===0?".64rem":".56rem",fontWeight:800,fontFamily:"'Plus Jakarta Sans'",letterSpacing:".01em",textTransform:"uppercase",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{seg.label}</span>
                    {hasCash&&<span className="cashB" style={{width:6,height:6,borderRadius:99,background:"#ff4444",border:"1.5px solid #fff",flexShrink:0}}/>}
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
                    <div style={{fontSize:".6rem",fontWeight:800,fontFamily:"'JetBrains Mono'",lineHeight:1.1}}>Rs {f$(seg.val)}</div>
                    <div style={{fontSize:".46rem",opacity:.8}}>{seg.pct.toFixed(1)}%</div>
                  </div>
                </button>
              </div>})}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:12}}><QS l="Day-1 Cash" v={`Rs ${f$(R.d1)}`}/><QS l="Monthly Outflow" v={fM(R.e+R.aR/12)}/>{R.tCash>0&&<div style={{gridColumn:"1/-1"}}><QS l="Cash Burden (off-books)" v={`Rs ${f$(R.tCash)}`} w/></div>}</div>
...
/* COMPONENTS */
function DRows({rows}){if(!rows)return null;return <div>{rows.map((d,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #f4f3f7",alignItems:"baseline",gap:8}}><div style={{display:"flex",alignItems:"center",gap:5,minWidth:0,flex:1}}>{d.cash&&<span style={{width:5,height:5,borderRadius:99,background:"#ba1a1a",flexShrink:0}}/>}<span style={{fontSize:".7rem",color:d.b?"#1a1c1e":d.w?"#ba1a1a":"#474556",fontWeight:d.b?800:500}}>{d.l}</span></div><span style={{fontSize:".7rem",fontFamily:"'JetBrains Mono'",fontWeight:d.b?800:600,color:d.w?"#ba1a1a":"#1a1c1e",whiteSpace:"nowrap",flexShrink:0}}>{typeof d.v==="number"?fF(d.v):d.v}</span></div>)}</div>}
